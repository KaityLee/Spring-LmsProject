...
public interface ReportRepository extends JpaRepository<Quiz_Report, Long> {
    ...
    public void reportPass(@Param("num") int num);
    
    @Modifying
    @Query("UPDATE quiz_report qr SET qr.reply = :reply WHERE qr.num = :num")
    public void reportReply(@Param("num") int num, @Param("reply") String reply);
}